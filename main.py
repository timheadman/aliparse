...
def get_price(url):
    ...
    if url != driver.current_url:
        logging.info(f'{time.strftime("%d-%m-%Y %H:%M:%S")}: '
    ...
    if not elements:
        elements = driver.find_elements(By.CLASS_NAME, 'Product_UniformBanner__uniformBannerBoxPrice__o5qwb')
    if not elements:
        elements = driver.find_elements(By.CLASS_NAME, 'snow-price_SnowPrice__mainS__ugww0l')
    if elements:
...
def print_report_table(db_price, db_sku):
    ...
    for name_id in name_id_dict.keys():
        ...
        for date_ in date_set:
            price.append(
                ''.join([str(db_row[2]) for db_row in db_price if db_row[0] == date_ and db_row[3] == name_id]))
...
if __name__ == '__main__':
    logging.basicConfig(encoding='utf-8', level=logging.INFO,
                        handlers=[
                            logging.FileHandler("aliparse.log"),
    ...
    try:
        ...
        logging.error(f'{time.strftime("%d-%m-%Y %H:%M:%S")}: '
                      f'Error connecting to MariaDB server: {e}.')
        sys.exit('Error connecting to MariaDB server.')

    cursor = connection.cursor(buffered=True)

    sql_query = 'SELECT id, url FROM sku WHERE id NOT IN ' \
    ...
    if len(data):
        ...
        for row in data:
            ...
            if current_price:
                sql_query = 'INSERT INTO price (sku_id, date, price) ' \
    ...
    sql_query = 'SELECT date, name, price, sku_id FROM price ' \
